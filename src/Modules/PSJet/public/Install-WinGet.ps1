...
function Install-WinGet {
    ...
    if (-not $wingetInstalled) {
        ...
        $isLegacyPS = ($psVersion.Major -lt 7)
        
        if ($isLegacyPS) {
            # Store the current value of $ProgressPreference
            $currentProgressPreference = $ProgressPreference
            
            # Set $ProgressPreference to 'SilentlyContinue' to improve Invoke-WebRequest performance
        ...
        try {
            ...
            if ($isLegacyPS) {
                # Restore the original value of $ProgressPreference
